...
- 路由模型：`/<platform>/<path>`。第一段是平台名，剥离后把剩余路径转发给该平台上游。
- 凭证模型：命名 `credential_pools`。多个 platform 可以共享同一池，也可以独立使用不同池。
...
`proxy.Handler` 按 platform 选择 runtime：

- `transport`：上游 base URL 与连接池。
- `provider`：凭证注入、限额解析、状态码分类。
- `keypool.Pool`：该 platform 引用的命名凭证池。
...
client request
  -> client auth
  -> split /<platform>/<path>
  -> acquire key from platform credential pool
...
- `workspaces.<name>` 自动转换为 `platforms.<name>.type = api_sports`。
- 顶层 `keys` 自动转换为 `credential_pools.default.keys`。

旧请求路径保持不变，例如 `/football/fixtures`。
